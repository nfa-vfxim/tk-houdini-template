...
class TkHoudiniTemplateHandler(object):
    ...
    def check_file(self):
        ...
        try:
            ...
            if 'untitled' in current_filepath:
                ...
                if difference != 0:
                    self.app.logger.debug("File is not empty, skipping template merging.")
                    return

                else:
                    self.app.logger.debug("File is empty, checking if pipeline step is valid to merge files.")
    ...
    def merge_file(self, step):
        try:
            ...
            template_filepath = os.path.join(template_directory, filename).replace(os.sep, '/')

            # Check if template file exists
            if os.path.isfile(template_filepath):
                try:
                    # Merging template file
                    self.app.logger.debug("Merging template file %s into current file." % template_filepath)
                ...
                self.app.logger.debug(
                    "Could not find template file %s, skipping template file merging." % template_filepath)
